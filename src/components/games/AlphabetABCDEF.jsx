...
// Letters to focus on: A B C D E F
const TARGET_LETTERS = ['A', 'B', 'C', 'D', 'E', 'F'];
...
const AlphabetABCDEF = () => {
    const { speak, cancel, speaking } = useTTS();
    const { getProgress, unlockLevel, saveLevel } = useProgress();
    const navigate = useNavigate();

    // Progress key: 'alphabet-abcdef'
    const progress = getProgress('alphabet-abcdef');
    ...
    useEffect(() => {
        saveLevel('alphabet-abcdef', currentLetterIndex + 1);
    }, [currentLetterIndex]);
        ...
        if (isCorrect) {
            speak("Yes!", () => {
                ...
                if (newRemaining.length > 0) {
                    ...
                    // Level complete
                    setTimeout(() => {
                        unlockLevel('alphabet-abcdef', currentLetterIndex + 2);
                        setMode('VICTORY');
                        confetti({
                            particleCount: 100,
                            spread: 70,
