...
// Letters to focus on: M N O P Q R
const TARGET_LETTERS = ['M', 'N', 'O', 'P', 'Q', 'R'];
...
const AlphabetMNOPQR = () => {
    const { speak, cancel, speaking } = useTTS();
    const { getProgress, unlockLevel, saveLevel } = useProgress();
    const navigate = useNavigate();

    // Progress key: 'alphabet-mnopqr'
    const progress = getProgress('alphabet-mnopqr');
    ...
    useEffect(() => {
        saveLevel('alphabet-mnopqr', currentLetterIndex + 1);
    }, [currentLetterIndex]);
        ...
        if (isCorrect) {
            speak("Yes!", () => {
                ...
                if (newRemaining.length > 0) {
                    ...
                    // Level complete
                    setTimeout(() => {
                        unlockLevel('alphabet-mnopqr', currentLetterIndex + 2);
                        setMode('VICTORY');
                        confetti({
                            particleCount: 100,
                            spread: 70,
