...
// Letters to focus on: G H I J K L
const TARGET_LETTERS = ['G', 'H', 'I', 'J', 'K', 'L'];
...
const AlphabetGHijkl = () => {
    const { speak, cancel, speaking } = useTTS();
    const { getProgress, unlockLevel, saveLevel } = useProgress();
    const navigate = useNavigate();

    // Progress key: 'alphabet-ghijkl'
    const progress = getProgress('alphabet-ghijkl');
    ...
    useEffect(() => {
        saveLevel('alphabet-ghijkl', currentLetterIndex + 1);
    }, [currentLetterIndex]);
        ...
        if (isCorrect) {
            speak("Yes!", () => {
                ...
                if (newRemaining.length > 0) {
                    ...
                    // Level complete
                    setTimeout(() => {
                        unlockLevel('alphabet-ghijkl', currentLetterIndex + 2);
                        setMode('VICTORY');
                        confetti({
                            particleCount: 100,
                            spread: 70,
